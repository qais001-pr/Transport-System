...
const getActiveVans = async (req, res) => {
  try {
    ...
    const vans = await pool.query(
      ...
      GROUP BY V.id, R.name, SB.start_time, SB.end_time
    ...
    const schedule = vans.rows.map((route) => {
      ...
      else if (now >= start && now < end) status = "on-route";
      return {
...
const verifyToStudent = async (req, res) => {
  try {
    ...
    const students = Array.isArray(child_ids) ? child_ids : [child_ids];
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "guard_id, child_ids, van_id, school_id, verification_type, latitude, longitude are required",
      });
    }
    ...
    for (const child_id of students) {
      try {
        ...
        insertedStudents.push(child_id);
      } catch (error) {
        if (error.code === "23505") {
          alreadyVerified.push(child_id);
        } else {
          throw error;
        }
      }
    ...
    return res.status(200).json({
      ...
      verified_students: insertedStudents,
      already_verified_students: alreadyVerified,
    });
  } catch (error) {
...
const verifyToAllStudents = async (req, res) => {
  try {
    ...
    const students = await pool.query(
      `
      SELECT C.id, B.van_id, C.branch_id
      FROM children C
    ...
    for (const student of students.rows) {
      await pool.query(
        `
        INSERT INTO guard_verifications
        (guard_id, child_id, van_id, branch_id, verification_type)
        VALUES ($1,$2,$3,$4,$5)
